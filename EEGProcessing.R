...
# Descrição: Análise e processamento de sinais de EEG 
# Data: 29 de Maio de 2021
# -------------------------------------------------------------------------

# Passo 1: Indicar o diretório de trabalho em que a Toolbox "read_Intan_RHD2000_file.R"
# está salva

setwd("D:/Ariana/Documentos/2021/PSB/EXEMPLO/")
...
# instalando as packages ainda não instaladas
...
for(c in 1:16){
  ...
  n_chan <- c # selecione o canal desejado (há 16 canais de EEG, você deve
  #                                         inserir um valor de 1 a 16)
  
  cat("Processando canal: ", n_chan,"\n\n")
  ...
  cat("Estimando as características \n\n")
  ...
  cat("Características estimadas para:", ifelse(str_sub(row.names(featureExtract),1,3)=="IMF",
                                                str_c(str_sub(row.names(featureExtract),1,3),
                                                      str_sub(row.names(featureExtract),5,-1)),
                                                row.names(featureExtract)), "\n\n")
